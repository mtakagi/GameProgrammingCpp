...
Enemy *Game::GetNearestEnemy(const Vector2 &pos) const {
    ...
    if (!mEnemies.empty()) {
        best = mEnemies[0];
        auto bestDistSq = (pos - mEnemies[0]->GetPosition()).LengthSq();

        for (auto i = 1; i < mEnemies.size(); i++) {
            auto newDistSq = (pos - mEnemies[i]->GetPosition()).LengthSq();
