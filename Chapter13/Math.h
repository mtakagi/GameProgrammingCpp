...
public:
    ...
    const static Vector3 NegUnitX;
    const static Vector3 NegUnitY;
    const static Vector3 NegUnitZ;
    const static Vector3 Infinity;
    const static Vector3 NegInfinity;
};
...
public:
    ...
    explicit Matrix3(const float inMat[3][3]) : mat() {
...
public:
    ...
    static Quaternion Slerp(const Quaternion& a, const Quaternion& b, const float f) {
        ...
        if (cosom < 0.9999f) {
            ...
            const auto invSin = 1.0f / Math::Sin(omega);
