...
void UIScreen::DrawTexture(const Shader *shader, const Texture *texture, const Vector2 &offset, const float scale, const bool flipY) {
    auto yScale = static_cast<float>(texture->GetHeight()) * scale;

    if (flipY) {
        yScale *= -1.0f;
    }

    const auto scaleMat = Matrix4::CreateScale(texture->GetWidth() * scale, yScale, 1.0f);
