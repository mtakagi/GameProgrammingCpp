...
const Vector3 Vector3::NegInfinity = Vector3(Math::NegInfinity, Math::NegInfinity, Math::NegInfinity);

Vector2 Vector2::Transform(const Vector2 &vec, const Matrix3 &mat, const float w) {
	Vector2 retVal;
...
Vector3 Vector3::TransformWithPerspDiv(const Vector3 &vec, const Matrix4 &mat, const float w) {
	Vector3 retVal;
	...
	auto transformedW = vec.x * mat.mat[0][3] + vec.y * mat.mat[1][3] + vec.z * mat.mat[2][3] + w * mat.mat[3][3];
...
void Matrix4::Invert() {
	...
	auto det = src[0] * dst[0] + src[1] * dst[1] + src[2] * dst[2] + src[3] * dst[3];
